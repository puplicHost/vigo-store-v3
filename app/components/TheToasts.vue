<template>
  <Teleport v-if="mounted" to="body">
    <div
      class="fixed top-8 end-8 z-[9999] flex flex-col gap-3 w-80 max-w-[min(100vw-2rem,20rem)] pointer-events-none"
      aria-live="polite"
    >
      <TransitionGroup
        tag="div"
      ...
      >
        <div
          v-for="notif in stripNotifications"
          :key="notif.id"
        ...
        >
          ...
          <button
            ...
            class="text-on-surface-variant/50 hover:text-error hover:bg-error/10 p-1.5 rounded-lg transition-colors flex-shrink-0"
          >
...
/** Floating strip shows all pending toasts until the user dismisses (no auto-hide). */
const stripNotifications = computed(() => notifications.value)
</script>
